...
describe("file-utils", () => {
  test("extracts a decoded file name from an HTTP URL", async () => {
    const { getFileNameFromUrl } =
      await import("../../../src/domain/document-server/file-utils.ts");

    expect(
      getFileNameFromUrl(
        "https://example.com/files/Quarterly%20Report.docx?download=1",
      ),
  ...
  test("falls back to basename for non-URL input", async () => {
    ...
    expect(getFileNameFromUrl("relative/path/presentation.pptx")).toBe(
      "presentation.pptx",
    );
  });
