...
describe("create-file", () => {
  test("builds blank file URLs", async () => {
    const { getBlankFileUrl } = await import("../../../src/tools/definitions/create-file.ts");
  ...
  test("adds the selected extension when the file name has no matching suffix", async () => {
    const { getFileName } = await import("../../../src/tools/definitions/create-file.ts");
  ...
  test("creates a document session for each supported file type", async () => {
    const { createCreateFileHandler } = await import("../../../src/tools/definitions/create-file.ts");
    const calls: unknown[] = [];
    ...
    await expect(handler({ fileName: "Document", fileType: "docx", locale: "en" })).resolves.toEqual({
      content: [],
      structuredContent: {
        sessionId: "session-1",
        documentServerBaseUrl: "https://document-server.example",
        config: { document: { title: "Document.docx" }, editorConfig: { mode: "edit" } },
        fileUrl: "blank://en/docx",
      },
    });
    await expect(handler({ fileName: "Sheet", fileType: "xlsx", locale: "de" })).resolves.toMatchObject({
      structuredContent: {
        config: { document: { title: "Sheet.xlsx" } },
        fileUrl: "blank://de/xlsx",
      },
    });
    await expect(handler({ fileName: "Deck", fileType: "pptx", locale: "fr" })).resolves.toMatchObject({
      structuredContent: {
    ...
    expect(calls).toEqual([
      { sessionId: "session-1", fileName: "Document.docx", fileUrl: "_data_", mode: "edit" },
      { sessionId: "session-1", fileName: "Sheet.xlsx", fileUrl: "_data_", mode: "edit" },
      { sessionId: "session-1", fileName: "Deck.pptx", fileUrl: "_data_", mode: "edit" },
    ]);
  });
  ...
  test("uses the default template locale when locale is omitted", async () => {
    ...
    await expect(handler({ fileName: "Document", fileType: "docx" })).resolves.toMatchObject({
      structuredContent: {
  ...
  test("registers the MCP app tool definition", async () => {
    const { createFile } = await import("../../../src/tools/definitions/create-file.ts");
